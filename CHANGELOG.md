...
- Tests: 17 in `tests/wezterm-honesty.test.ts` plus 2 in `tests/wake-fallback.test.ts` (pane resolution incl. unknown ancestry, rejoin clearing through room and manager, process-tree check with the field case, macOS paths, pid reuse, CIM dates, fallback and its error precedence, precision-aware ordering, etime and process-table parsing, the fallback guard at both checkpoints, the no-auto-start guard). Suite 136.
