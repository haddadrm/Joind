...
- Tests: 18 in `tests/wake.test.ts` (classification, base URL, serialization by terminal, skip and moved results, retry and attempt accounting, warn-once per room until forget, transient rate limit, presence event and lastPostAt on the room, stale outcomes, proof-of-life reset, shared-key serialization, execution-time generations, release and reclaim, composite identity, destroy). Suite 112.
