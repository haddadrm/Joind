...
- Tests: 14 in `tests/wezterm-honesty.test.ts` (pane resolution incl. unknown ancestry, rejoin clearing through room and manager, process-tree check with the field case, macOS paths, pid reuse, CIM dates, fallback and its error precedence, the no-auto-start guard). Suite 131.
