...
### Codex gate round 9 (2 Medium, closed)
- **`chat_leave` forgets the session only after the departure.** When the departure fails (the release record cannot be written), the session record, the member, its binding and its home registration all stay, and a plain `chat_leave` retry reaches the same registration.
- **Member registrations are write-ahead too.** The member record (`<room>.releases.json`, now `{ releases, intents }`; the round-7 array still reads) gets an intent (the name and this server's registration id) BEFORE any member registration goes to the home: recovery re-registration, a join, and an abandoned join's restore. The reply turns the intent into the member's shadow when the member is still here, or into release debt when it left. A record that cannot be written aborts the attempt (a join answers 503). An intent left by a lost reply or a restart is settled at recovery: registered again (idempotent at the home for the same host registration) to learn its id, then released; a 404 or 409 there drops it. An abandoned join's registration always becomes debt, cleared by a 404 once the newer registration replaced it.
- 3 tests in `tests/linked-servers-gate9.test.ts`, all failing on 1e3ed7b: `chat_leave` through the MCP callbacks on a real registry (a failed departure, then a plain retry that leaves); a recovery reply landing after the member left, with the record unwritable and the release failing, released after a restart through recovery; and a registration whose intent cannot be written is not sent. The round-8 test reads the new record layout. Suite 415.

### Codex gate round 10 (3 Medium, 1 Low, closed by a restructure): the member lifecycle as one write-ahead record
- **`src/member-state.ts`.** One record per member of a remote room, in one file per room, `<room>.members.json`: `{ members: [{ name, live, unconfirmed, releasesOwed, seq }], legacyHomeReleases }`. Every id is THIS server's registration id for the member (the one the home keeps as the hosted registration). `live` is the member here now, `unconfirmed` an id sent whose outcome is unknown, `releasesOwed` ids the home may still hold that belong to no member here. Every transition is written atomically (a temp file, then a rename) before the remote call and again on its answer; a failed write throws `MemberStateError` and the transition does not happen (a join answers 503, a deliberate leave fails and keeps the member).
- **Migration from round 9.** When no members file exists, `<room>.releases.json` is read once: its `intents` (`{ name, hosted }`) become owed ids, and its `releases` (which carried the HOME's ids) become `legacyHomeReleases`, released by the home id (`/api/peer/leave` with `registration`) until settled. The round-7 bare array is read the same way. The next write creates the members file; the old file is then ignored.
- **The home releases by the peer's id.** `/api/peer/leave` also accepts `{ room, name, hostedRegistration }` and releases whatever the home holds for that host and that id. An old id is never registered again to learn anything: obsolete ids are released directly; 404 retires them, link errors keep them, other refusals keep them.
- **Joins.** The id is recorded as `unconfirmed` before the request (a previous unconfirmed id that is not live becomes owed); a definitive refusal forgets it at once; a link error keeps it for recovery; the commit makes it `live` (the former live id owed); an abandon owes it, and the restore of the member here goes through the same confirmation.
- **Replies are applied only to the member they were for.** A reply (join, restore, recovery) is re-checked after its await: it becomes live only if the member here still has that id; otherwise it is owed. A departure writes its id as owed before the member is removed; its release waits for the name's lock.
- **Recovery** runs for any mirror with a member here, a member record, or viewer state. Per name, under the lock: (a) a member here: its id is recorded live, registered, and confirmed as above; a live id with no member here (a restart) becomes owed; (b) every other id, unconfirmed or owed, is released by id. The live registration is reconciled first, so a mention during recovery reaches the live member.
- 4 tests in `tests/linked-servers-gate10.test.ts`, one per finding, all failing on a99e988, through the production paths: an intent-only room (a first join accepted by the home, reply lost) cleaned by recovery after a restart; an OLD join's lost reply, then NEW joined, then recovery with its first registration reply paused and a real mention from the home reaching NEW (two real servers); a restore reply landing after the member departed, released; 25 joins refused with 409 leaving no record. The round 7 to 9 tests follow the new home API (their fake homes release by the host's id) and the new file; their scenarios are unchanged and green. Suite 419.
