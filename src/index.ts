...
/** Test and embedding knobs; the command line uses none of them. */
export interface StartOptions {
  /** How long a silent linked peer may stay silent before its rooms are told
...
/**
 * Start one Joind server. The command line calls this once (see the bottom
 * of this file); tests start several in one process on loopback ports.
 * Port 0 picks a free port; the handle reports the real one.
 */
export async function startJoind(CONFIG: JoindConfig, startOptions: StartOptions = {}): Promise<JoindHandle> {
  ...
  // --- Linked servers ---
  // This server's links (peers it mirrors rooms from, and hosts members of
  // its own rooms for) and its home side (the peer routes).
  const linkRegistry = new LinkRegistry(CONFIG.links, {
    selfName: INSTANCE_NAME,
    linksDir: join(DATA_DIR, "links"),
    manager,
    ...(startOptions.link ?? {}),
  });
  ...
  const peerHub = new PeerHub({
    ...
    getPersistedRole: (name) => agentRoles[name],
    peerGraceMs: startOptions.peerGraceMs ?? CONFIG.presenceGraceMs,
    monitorEveryMs: startOptions.peerMonitorEveryMs,
  });
  peerHub.attach([
    { on: (e, fn) => taskStore.on(e, fn), eventName: "task" },
    { on: (e, fn) => reactionStore.on(e, fn), eventName: "reaction" },
    { on: (e, fn) => editStore.on(e, fn), eventName: "edit" },
  ]);
  ...
  function pushNotification(classified: Classified | null, conversationId: string): void {
    ...
    const name = manager.listAllRoomMetas().find((c) => c.id === conversationId)?.name;
  ...
  manager.on("room", (event) => {
    ...
    // Hook session engine into message stream (local rooms only: a remote
    // room's workflow runs on its home server)
    if (event.type === "message" && event.data.sender !== "system" && !manager.isRemote(event.conversationId)) {
  ...
  // Linked servers: link state, the undelivered queue, and local-only lines
  // of remote rooms. Envelopes are { type, conversationId?, data } like every
  // other event; a pending message follows DM visibility like a message.
  linkRegistry.on("link", (info) => {
  ...
  linkRegistry.on("notice", (n: MirrorNotice) => {
    ...
    for (const client of wss.clients) {
      if (client.readyState !== WebSocket.OPEN) continue;
      if (n.type === "pending") {
        const probe: ChatMessage = { id: 0, sender: n.data.sender, text: "", timestamp: n.data.queuedAt, ...(n.data.to ? { to: n.data.to } : {}) };
        if (!visibleToViewer(probe, clientNames.get(client))) continue;
      }
      client.send(msg);
    }
  });
    ...
    ws.send(
      JSON.stringify({
        ...
        data: {
          ...
          messages: activeRoom ? viewMessages(activeRoom, 100, viewerName) : [],
  ...
  function viewMessages(room: ReturnType<typeof manager.getRoom> & object, limit: number, viewer: string | undefined): ChatMessage[] {
    return room instanceof MirrorRoom ? room.readForView(limit, viewer) : room.read(undefined, limit, undefined, viewer);
  }

  /** Linked servers in the UI payloads: link states, remote rooms, and the
   *  viewer's undelivered messages. */
  function linkPayload(viewer: string | undefined) {
    return {
      links: linkRegistry.infos(),
      remoteConversations: linkRegistry.remoteConversations(),
      pending: linkRegistry.pendingFor(viewer),
    };
  }
  ...
  // --- Linked servers: the peer routes (token-authenticated per link) ---
  app.use("/api/peer", peerHub.router());
  ...
  app.post("/api/conversations/select", express.json(), async (req, res) => {
    ...
    res.json({
      ...
      agents: room?.who() ?? [],
      ...(room instanceof MirrorRoom ? { pending: room.pendingFor(webViewer()), remote: { server: room.server, state: linkRegistry.get(room.server)?.info().state ?? "down" } } : {}),
    });
  });

  // Delete an undelivered message of a remote room (the viewer's own only).
  app.post("/api/pending/delete", express.json(), (req, res) => {
    const { conversation, clientId, token } = (req.body ?? {}) as { conversation?: string; clientId?: string; token?: string };
    if (!webAuthorized(token)) { res.status(403).json({ error: "unauthorized" }); return; }
    const viewer = webViewer();
    if (!viewer) { res.status(409).json({ error: "no viewer registered" }); return; }
    if (!conversation || !clientId) { res.status(400).json({ error: "conversation and clientId required" }); return; }
    const room = manager.getRoom(conversation);
    if (!(room instanceof MirrorRoom)) { res.status(404).json({ error: "No undelivered messages here: not a remote room" }); return; }
    const r = room.deleteUndelivered(clientId, viewer);
  ...
  app.post("/api/agent/join", express.json(), async (req, res) => {
    ...
    res.json({
      ok: true,
      registration,
      conversation: { id: convId, name: meta?.name ?? convId, ...(room instanceof MirrorRoom ? { server: room.server, remote: true } : {}) },
  ...
  app.post("/api/agent/send", express.json(), async (req, res) => {
    ...
    if (ctx.room instanceof MirrorRoom) {
      // Written through to the home server; queued (202) while the link is down.
      try {
        const r = await ctx.room.writeThrough(sender, text, { replyTo, choices, askFor: typeof askFor === "string" ? askFor : undefined, to: recipients });
        if (r.status === "sent") {
          const msg = r.message;
          res.json({ id: msg.id, sender: msg.sender, text: msg.text, choices: msg.choices, to: msg.to, ask: msg.ask });
        } else {
          res.status(202).json({ queued: true, clientId: r.clientId, reason: r.reason, text: `Queued, not sent yet: ${r.reason}. It goes out in order when the link returns; POST /api/agent/pending/delete with this clientId deletes it before then.` });
        }
      } catch (err) {
        res.status(err instanceof PeerRefusedError ? err.status : 502).json({ error: (err as Error).message });
      }
  ...
  // Delete one of your own undelivered messages in a remote room.
  app.post("/api/agent/pending/delete", express.json(), (req, res) => {
    const { name, clientId, pid, paneId } = (req.body ?? {}) as { name?: string; clientId?: string; pid?: number; paneId?: number };
    if (!name || !clientId) { res.status(400).json({ error: "name and clientId required" }); return; }
    const ctx = agentRoom(name, res, pid, paneId, orcaOf(req), weztermGuiOf(req), registrationOf(req));
    if (!ctx) return;
    if (!(ctx.room instanceof MirrorRoom)) { res.status(404).json({ error: "No undelivered messages here: not a remote room" }); return; }
    const r = ctx.room.deleteUndelivered(clientId, name);
    if (!r.ok) { res.status(r.status).json({ error: r.error }); return; }
    res.json({ ok: true });
  });
  ...
  // The base URL wake prompts name: the address this server really binds.
  manager.injectBaseUrl = injectBaseUrlFor(HOST, boundPort);
  for (const m of manager.listRemote()) m.room.injectBaseUrl = manager.injectBaseUrl;
  for (const conv of manager.listConversations()) {
    const r = manager.getRoom(conv.id);
    if (r) r.injectBaseUrl = manager.injectBaseUrl;
  }
  linkRegistry.start();
  peerHub.startMonitor();
  {
    const shown = HOST === "0.0.0.0" ? "127.0.0.1" : HOST;
    console.log(`\n  ╔═══════════════════════════════════════╗`);
  ...
  return {
    ...
    close: async () => {
      ...
      wss.close();
      for (const s of mcpSessions.values()) await s.transport.close().catch(() => undefined);
      await new Promise<void>((resolve) => {
        httpServer.close(() => resolve());
        httpServer.closeAllConnections();
      });
      for (const m of manager.listRemote()) m.room.destroy();
...
// --- Command line ---
// Started as a program (node dist/index.js), never when imported.
const isMain = (() => {
...
if (isMain) {
  ...
  startJoind(config).then((handle) => {
    for (const sig of ["SIGINT", "SIGTERM"] as const) {
      process.once(sig, () => { void handle.close().finally(() => process.exit(0)); });
    }
  }, (err: unknown) => {
    console.error(`  Joind failed to start: ${err instanceof Error ? err.message : String(err)}`);
    process.exit(1);
  });
}
