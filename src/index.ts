...
app.post("/api/join", express.json(), async (req, res) => {
  ...
  const joinToken = manager.beginJoin(name, ConversationManager.joinTerminalKeys(pid, requestedPane), convId);
  ...
  if (!manager.joinIsCurrent(joinToken)) { res.status(409).json({ error: "Join superseded by a newer join or a departure for this name" }); return; }
...
app.post("/api/agent/join", express.json(), async (req, res) => {
  ...
  if (!manager.getRoom(convId)) { res.status(404).json({ error: "Conversation not found" }); return; }
  const joinToken = manager.beginJoin(name, ConversationManager.joinTerminalKeys(pid, weztermPaneId), convId);
